...
### Get digest
```sh
curl -v --user ${user}:${password} <registry_url>/v2/<repo>/manifests/<tags> -H 'Accept: application/vnd.docker.distribution.manifest.v2+json'
```
### Delete Image
```sh
curl -v --user ${user}:${password} -X DELETE <registry_url>/v2/<repo>/manifests/<digest>
```
